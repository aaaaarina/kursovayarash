...
Lmax = 10001  # Максимальное значение элемента массива
...
def main():
    ...
    while True:  # Бесконечный цикл для меню
        ...
        if choice == "0":
            ...
            n = int(input("Введите размер массива: "))  # Ввод размера массива
            ...
            item = int(input("Введите элемент для поиска: "))  # Ввод искомого элемента
            indices = binary_search_all(CopyA, item)  # Двоичный поиск
            if indices:
                print("Элемент", item, "найден в позициях:", ", ".join(map(str, indices)))  # Вывод результатов поиска
            else:
                print("Элемент не найден")  # Вывод сообщения об отсутствии элемента
